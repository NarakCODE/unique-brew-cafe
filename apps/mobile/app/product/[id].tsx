...
export default function ProductDetailScreen() {
  ...
  const dotCount = Math.max(3, galleryImages.length);
    ...
    <>
      ...
      <View className="flex-1 bg-background">
        {isLoading ? (
          <>
            <ScreenLayout
              contentClassName="gap-7 px-4"
              topInsetOffset={12}
              bottomInsetOffset={168}
            >
              <ProductDetailLoadingState />
            </ScreenLayout>
            <BottomActionBarSkeleton bottomInset={insets.bottom} />
          </>
        ) : isError || !data ? (
          <ScreenLayout
            className="px-4"
          ...
          <>
            ...
            >
                      ...
                      {imageUri.trim() ? (
                        ...
                        >
                          ...
                          <Text className="mt-3 text-sm text-muted-foreground">
                            {PRODUCT_DETAIL_COPY.imageFallback}
                          </Text>
                        </View>
                      )}
                    </View>
              ...
              <View className="rounded-[22px] border border-border bg-card px-4 py-4">
                <View className="flex-row items-center justify-between">
                  <Text className="text-base font-medium text-muted-foreground">
                    {PRODUCT_DETAIL_COPY.subtotal}
                  </Text>
                  <Text

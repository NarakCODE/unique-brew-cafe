...
import { z } from "zod";
import { FormatStyleType } from "@/types";
...
export function isActivePathname(
    ...
    if (
        typeof basePathname !== "string" ||
        typeof currentPathname !== "string"
    ) {
        throw new Error(
            "Both basePathname and currentPathname must be strings"
        );
    }
...
export function formatNumberToCompact(value: number) {
    return new Intl.NumberFormat("en", {
        notation: "compact",
